...
const Stylis = _interopDefault(require('stylis/stylis.min'));
const _insertRulePlugin = _interopDefault(require('stylis-rule-sheet'));
const React = require('react');

const React__default = _interopDefault(React);
const unitless = _interopDefault(require('@emotion/unitless'));
const reactIs = require('react-is');
const memoize = _interopDefault(require('memoize-one'));
const PropTypes = _interopDefault(require('prop-types'));
const ReactDOM = _interopDefault(require('react-dom'));
const validAttr = _interopDefault(require('@emotion/is-prop-valid'));
...
const TYPE_STATICS = (_TYPE_STATICS = {}, _TYPE_STATICS[reactIs.ForwardRef] = {
  $$typeof: true,
  render: true
}, _TYPE_STATICS);
...
const classNameUsageCheckInjector = (function (target) {
  ...
  target.componentDidMount = function componentDidMount() {
    ...
    if (node && node.nodeType === 1 && !classNames.every((className) => node.classList && node.classList.contains(className)) && !node.querySelector(selector)) {
      // eslint-disable-next-line no-console
...
const displayName = _props$forwardedCompo.displayName;

        
const foldedComponentIds = _props$forwardedCompo.foldedComponentIds;
